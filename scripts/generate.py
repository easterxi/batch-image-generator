...
def validate_server(server):
    ...
    if missing: raise RuntimeError("Missing ComfyUI node types: "+", ".join(missing))
...
def expand_prompt(template,traits,rng):
    ...
    def repl(m):
        ...
        vals=traits[k]
        if not isinstance(vals,list) or not vals: raise ValueError(f"Trait {k} must be a non-empty list")
        v=rng.choice(vals); used[k]=v; return str(v)
    return re.sub(r"\{([A-Za-z0-9_]+)\}",repl,template),used

def prep_reference(src,dst,w,h,mode,low,high):
    if not src.exists(): raise FileNotFoundError(f"Reference image not found: {src}")
...
def queue(server,wf):
    ...
    if "prompt_id" not in x: raise RuntimeError(f"No prompt_id returned: {x}")
    return x["prompt_id"]

def wait_history(server,pid,poll,timeout):
    start=time.time(); url=server.rstrip("/")+f"/history/{pid}"
    while True:
        if time.time()-start>timeout: raise TimeoutError(f"Timed out waiting for {pid}")
        try: h=http_json(url,timeout=30)
        except Exception: time.sleep(poll); continue
        if pid in h:
            entry=h[pid]; st=entry.get("status",{})
            if st.get("completed") is True or st.get("status_str") in {"success","error"}: return entry
        time.sleep(poll)
...
def saved_image(entry):
    ...
    for o in outs.values():
        for im in o.get("images",[]):
            if im.get("type")=="output": return im
...
def build(template,cfg,pos,neg,seed,index,ckpt,cn):
    ...
    a=wf[NODE["apply"]]["inputs"]; a["strength"]=float(cfg["controlnet_strength"]); a["start_percent"]=float(cfg["controlnet_start"]); a["end_percent"]=float(cfg["controlnet_end"])
    l=wf[NODE["latent"]]["inputs"]; l["width"]=int(cfg["width"]); l["height"]=int(cfg["height"]); l["batch_size"]=1
    s=wf[NODE["sampler"]]["inputs"]; s.update({"seed":int(seed),"steps":int(cfg["steps"]),"cfg":float(cfg["cfg"]),"sampler_name":cfg["sampler_name"],"scheduler":cfg["scheduler"],"denoise":float(cfg["denoise"])})
    wf[NODE["save"]]["inputs"]["filename_prefix"]=f"batch/{cfg['filename_prefix']}_{index:06d}"
    return wf

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--config",default="config/config.json"); ap.add_argument("--count",type=int,required=True)
    ...
    print("Preparing reference..."); prep_reference(ref,comfy/"input"/"control_reference.png",int(cfg["width"]),int(cfg["height"]),cfg["reference_preprocess"],cfg["canny_low_threshold"],cfg["canny_high_threshold"])
    done=manifest_done(manifest); rng=random.Random()
    print(f"Target count: {args.count}\nOutput: {out}\nReference: {ref}\nCheckpoint: {ckpt}\nControlNet: {cn}\n")
    for i in range(1,args.count+1):
        target=out/f"{cfg['filename_prefix']}_{i:06d}.png"
        if not args.force and (i in done or target.exists()): print(f"[{i:06d}/{args.count:06d}] skip"); continue
        prompt,used=expand_prompt(pos_t,traits,rng); seed=seed_for(cfg["seed_mode"],cfg["base_seed"],i,rng); wf=build(template,cfg,prompt,neg,seed,i,ckpt,cn); pid=""
