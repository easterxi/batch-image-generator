...
def prep_reference(src, dst, w, h, mode, low=100, high=200):
    ...
    with Image.open(src) as source:
        ...
        if mode == "canny":
            ...
            # The OpenPose preprocessor may preserve aspect ratio internally.
            # Force the exact ControlNet canvas expected by the latent.
            im = ImageOps.fit(
                im,
                (w, h),
                method=Image.Resampling.LANCZOS,
            ...
            raise ValueError(
                "reference_preprocess must be 'canny', 'openpose', or 'none'."
            )
...
def log(path, row):
    fields = [
        "index", "status", "filename", "seed", "prompt_id",
        "timestamp_utc", "traits_json", "prompt", "error",
    ]
    exists = path.exists()
...
def build(template, cfg, pos, neg, seed, index, ckpt, cn):
    ...
    l["width"] = int(cfg["width"])
    l["height"] = int(cfg["height"])
    l["batch_size"] = 1
    ...
    wf[NODE["save"]]["inputs"]["filename_prefix"] = (
        f"batch/{cfg['filename_prefix']}_{index:06d}"
    )
...
def main():
    ...
    for i in range(1, args.count + 1):
        ...
        print(
            f"[{i:06d}/{args.count:06d}] "
            f"seed={seed} traits={json.dumps(used, ensure_ascii=False)}",
            flush=True,
        )
